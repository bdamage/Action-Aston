...
export const SKIN_OPTIONS: SkinOption[] = [
  ...
  { name: 'Nova Red',   description: 'Fiery red tint',   color: '#ff7070', cost: 180 },
  { name: 'Emerald',    description: 'Emerald green',    color: '#6bffb8', cost: 180 },
  { name: 'Solar Gold', description: 'Golden chrome',    color: '#ffd060', cost: 280 },
];
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        purchasedSkins:  Array.isArray(parsed.purchasedSkins) ? parsed.purchasedSkins : [0],
      };
    }
  } catch { /* ignore */ }
  return { weaponLevel: 0, shieldLevel: 0, boostLevel: 0, activeSkinIndex: 0, purchasedSkins: [0] };
}
...
export interface GarageStore extends PersistedGarage {
  ...
  buySkin:        (skinIndex: number, spendCoins: (amount: number) => boolean) => boolean;
...
export const useGarageStore = create<GarageStore>((set, get) => {
  ...
  return {
    ...
    upgradeWeapon: (spendCoins) => {
      ...
      const cost = WEAPON_TIERS[nextLevel].cost - WEAPON_TIERS[weaponLevel].cost;
    ...
    upgradeShield: (spendCoins) => {
      ...
      const cost = SHIELD_TIERS[nextLevel].cost - SHIELD_TIERS[shieldLevel].cost;
      if (!spendCoins(cost)) return false;
    ...
    buySkin: (skinIndex, spendCoins) => {
      ...
      const updated = { ...get(), purchasedSkins: newPurchasedSkins, activeSkinIndex: skinIndex };
      saveGarage(updated);
